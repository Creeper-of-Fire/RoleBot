    ...
    async def update_guide_embed_task(self):
        ...
        if not all(guide_info.get(k) for k in ["guild_id", "channel_id", "post_id"]):
            ...
            if self.update_guide_embed_task.current_loop > 0:  # 首次不提示
                self.logger.warning("幻化指引配置不完整，无法更新缓存Embed。")
            return
